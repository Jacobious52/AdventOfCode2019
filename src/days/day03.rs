...
fn parse_input(s: &str) -> Vec<Vec<(String, usize)>> {
    s.lines().map(|l| l.split(',').map(|w| (w[..1].to_string(), w[1..].parse().unwrap())).collect()).collect()
}
    ...
    let problem = Problem::<_, usize>::new(|wires: Vec<Vec<(String, usize)>>| {
        ...
        for i in 0..wire_points.len() {
            for j in i+1..wire_points.len() {
                ...
                let min_point = both.min_by(|x, y| (x.0.abs() + x.1.abs()).cmp(&(y.0.abs() + y.1.abs()))).unwrap_or(&&(0, 0));
    ...
    let problem = Problem::<_, usize>::new(|wires: Vec<Vec<(String, usize)>>| {
        ...
        for i in 0..wire_points.len() {
            for j in i+1..wire_points.len() {
                ...
                let b: HashSet<_> = wire_points[j].keys().collect();
                let both = a.intersection(&b);
                let dist = both.map(|x| wire_points[i][x]+wire_points[j][x]).min().unwrap_or(0);
                if dist < min {
                    min = dist;
                }
            }
